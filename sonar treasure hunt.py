    ...
    while True:
        ...
        if len(move) == 2 and move[0].isdigit() and move[1].isdigit() and isOnBoard(int(move[0]), int(move[1])):
            if [int(move[0]), int(move[1])] in previousMoves:
                print('You already have a sonar there.')
                continue
...
while True:
    ...
    while sonarDevices > 0:
        ...
        moveResults = makeMove(theBoard, theChests, x, y)
        if moveResults == False:
            continue
        else:
            if moveResults == 'You have found a sunken treasure chest!':
        ...
        if len(theChests) == 0:
            print('You have found all the sunken tresure chests! Congratulations and good game')
            break

        sonarDevices -= 1

    if sonarDevices == 0:
        print('You\'ve run out of sonar devices! Now we have to turn the ship around and head')
